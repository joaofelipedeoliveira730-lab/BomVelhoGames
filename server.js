...
function publicUser(u) { return { id:u.id, username:u.username, role:u.role, coins:Number(u.coins||0), xp:Number(u.xp||0), level:Number(u.level||1), wins:Number(u.wins||0), losses:Number(u.losses||0), gamesPlayed:Number(u.games_played||0), country:String(u.country||'BR').toUpperCase() }; }
...
app.get('/api/rank',async(req,res)=>{
  ...
  const r=await pool.query(`
    SELECT u.username,u.level,u.xp,u.coins,u.wins,u.games_played,u.country,
           p.avatar,
           CASE WHEN u.games_played>0 THEN ROUND((u.wins::numeric/u.games_played::numeric)*100,1) ELSE 0 END AS win_rate
    FROM users u
    LEFT JOIN profiles p ON p.user_id=u.id
    WHERE u.role<>'CEO' AND u.role<>'banned' AND LOWER(u.username)<>'ceovelho'
    ORDER BY u.wins DESC,u.xp DESC,u.level DESC,u.games_played DESC,u.username ASC
    LIMIT 10
  `);
  ...
  const players=r.rows.map(p=>{
    const id=idByName.get(String(p.username).toLowerCase());
    const level=Number(p.level||1);
    const prestige=Math.min(5,Math.floor(Math.max(0,level-1)/50));
    return {username:p.username,level,xp:Number(p.xp||0),coins:Number(p.coins||0),wins:Number(p.wins||0),gamesPlayed:Number(p.games_played||0),winRate:Number(p.win_rate||0),country:String(p.country||'BR').toUpperCase(),avatar:p.avatar||{},online:id?onlineIds.has(id):false,prestige};
  });
...
function publicPlayer(p,i){return {userId:publicPlayerId(p,i),username:p.username,role:'user',connected:p.connected,cardCount:p.hand?.length||0,avatar:p.avatar||null,level:Number(p.level||1),prestige:Math.min(5,Number(p.prestige||Math.floor(Math.max(0,Number(p.level||1)-1)/50))) };}
