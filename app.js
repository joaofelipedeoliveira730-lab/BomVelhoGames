...
function updateUserUI(){const u=state.user;if(!u)return;$('#coinValue').textContent=formatNum(u.coins);$('#levelValue').textContent=u.level;$('#heroName').textContent=u.username;$('#winsValue').textContent=u.wins||0;$('#xpValue').textContent=formatNum(u.xp);$('#profileName').textContent=u.username;$('#profileLevel').textContent=u.level;$('#profileWins').textContent=u.wins||0;$('#profileGames').textContent=u.gamesPlayed||0;$('#accountInfo').innerHTML=`<b>${escapeHtml(u.username)}</b><br>Cargo: ${escapeHtml(u.role)}<br>🪙 ${formatNum(u.coins)} • ⭐ ${formatNum(u.xp)} XP`;const pct=Math.min(100,Math.max(0,((u.xp-(u.level>1?xpForLevelClient(u.level):0))/Math.max(1,xpForLevelClient(u.level+1)-(u.level>1?xpForLevelClient(u.level):0)))*100));$('#xpBar').style.width=pct+'%';const title=itemName(state.profile?.avatar?.title)||'INICIANTE'; $('#btnCEO')?.classList.toggle('hidden',u.role!=='CEO');$('#profileTitle').textContent=title.toUpperCase(); $('#profileTitle').classList.toggle('title-owner',state.profile?.avatar?.title==='title_owner');$('#customNamePreview').textContent=u.username;$('#customTitlePreview').textContent=title.toUpperCase(); $('#customTitlePreview').classList.toggle('title-owner',state.profile?.avatar?.title==='title_owner');$('#avatarMiniFace').textContent='🙂';$('#heroCharacterLabel')&&($('#heroCharacterLabel').textContent=u.username);$('#shopCoinValue')&&($('#shopCoinValue').textContent=formatNum(u.coins));renderCharacter('#heroCharacterLarge',state.profile?.avatar||{});}
...
function loadAchievementsPreview(){const a=[['🏆','Primeira Vitória'],['🃏','Rei das Cartas'],['🌎','Primeiro Online'],['🎒','Colecionador']];$('#achievementPreview').innerHTML=a.map(x=>`<div class="achievement-chip"><span>${x[0]}</span><b>${x[1]}</b></div>`).join('');}
...
function renderSoloGame(){const g=state.solo;if(!g)return;$('#roundText').textContent='1 / 1';showTurnNotice(g.turn==='player'?'SUA VEZ!':'AGUARDE...',g.turn==='player');$('#discardPile').className=`uno-card card-${g.color} big-card`;$('#discardPile').textContent=g.discard.value;$('#colorIndicator').textContent=SOLO_NAMES[g.color];$('#deckCount').textContent=g.deck.length;$('#opponents').innerHTML=`<div class="opponent-card"><div class="opponent-avatar">🙂</div><div><b>Oponente</b><small>${g.bot.length} cartas</small></div><div class="mini-hand">${Array.from({length:Math.min(g.bot.length,7)}).map(()=>'<span class="back-mini">UNO</span>').join('')}</div></div>`;$('#playerHand').innerHTML=g.player.map((c,i)=>`<button class="uno-card card-${c.color} hand-card" data-index="${i}"><i>${c.value}</i><span>${c.value}</span><em>${c.type==='number'?'NÚMERO':c.type.toUpperCase()}</em></button>`).join('');$$('#playerHand .hand-card').forEach(b=>b.onclick=()=>attemptSoloCard(Number(b.dataset.index)));}
...
function renderAchievements(){const arr=[['first_win','🏆','Primeira Vitória','Vença sua primeira partida.'],['uno_10','🃏','Rei das Cartas','Vença 10 partidas de UNO.'],['online_first','🌎','Primeiro Online','Finalize uma partida online.'],['collector','🎒','Colecionador','Desbloqueie 10 itens.'],['level_10','⭐','Nível 10','Alcance o nível 10.'],['level_25','💎','Nível 25','Alcance o nível 25.']];$('#inventoryContent').innerHTML=`<div class="achievement-list">${arr.map(a=>`<div class="achievement-card"><span>${a[1]}</span><div><b>${a[2]}</b><small>${a[3]}</small></div><em>EM PROGRESSO</em></div>`).join('')}</div>`;}
