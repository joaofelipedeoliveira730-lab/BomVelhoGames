...
async function requireMapResource(mapId){const m=refs.maps.find(x=>x.id===mapId); if(!m)return false; const ok=await isResourceCached('/'+m.resource); if(!ok){toast('Mapa ainda não está disponível neste dispositivo. Baixe os recursos novamente.','error',6000);return false;}return true;}
