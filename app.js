...
const API = '/api';
const VERSION = '20260817-UNO50-CLASSIC';
...
const MAPS = [
  ...
  ['map_neon_city','Cidade Neon','neon'],['map_forest','Floresta','forest'],['map_desert','Deserto Dourado','desert'],
  ['map_ice','Montanha Congelada','ice'],['map_space','Estação Espacial','space'],['map_math_dimension','Dimensão Portal','math'],['map_ceo','Sala do CEO','ceo']
...
const SAVED_PLATFORM = localStorage.getItem('uv_platform_version')===VERSION ? localStorage.getItem('uv_platform') : null;
const state = {
  ...
  solo:null, pendingChallenge:null, pendingSoloCard:null, pendingCard:null, muted:false, platform:SAVED_PLATFORM, orientation:localStorage.getItem('uv_orientation')||null, classicCount:1, classicTimer:null, classicSearchStarted:false
};
...
function toast(message,type='info',ms=3000){const box=$('#toastContainer');if(!box)return;const el=document.createElement('div');el.className=`toast ${type}`;el.innerHTML=`<span>${type==='error'?'':type==='success'?'':'ℹ'}</span><div>${escapeHtml(message).replace(/\n/g,'<br>')}</div>`;box.appendChild(el);setTimeout(()=>el.classList.add('out'),Math.max(300,ms-350));setTimeout(()=>el.remove(),ms);}
...
async function continueAfterPlatform(){
  if(!state.orientation){hide('#authScreen');hide('#appScreen');show('#platformScreen');show('#orientationOptions');return;}
  hide('#platformScreen');
  try{
...
async function choosePlatform(platform){
  ...
  show('#orientationOptions');
}
async function chooseOrientation(orientation){
  state.orientation=orientation;
  localStorage.setItem('uv_orientation',orientation);
  document.body.dataset.orientation=orientation;document.documentElement.dataset.orientation=orientation;
  hide('#orientationOptions');Sound.init();
  await continueAfterPlatform();
}
function updateOrientationGuard(){hide('#orientationGuard');document.body.dataset.orientation=state.orientation||'landscape';}
...
function bindEvents(){
  ...
  on('#btnOrientationLandscape','click',()=>chooseOrientation('landscape'));
  on('#btnOrientationPortrait','click',()=>chooseOrientation('portrait'));
  on('#btnAuthExit','click',()=>{hide('#authScreen');show('#platformScreen');});
  ...
  on('#btnSettingsHome','click',()=>navigate('settings'));
  on('#btnHistoryHome','click',openHistory);
  ...
  on('#btnCancelClassicSearch','click',cancelClassicSearch);
  on('#btnCeoSearch','click',ceoSearch);
  on('#btnSaveSeason','click',saveSeason);
  on('#btnCeoPause','click',()=>ceoCommand('/paralisaruno Jogo pausado pelo CEO.'));
  on('#btnCeoResume','click',()=>ceoCommand('/desparalisaruno'));
  on('#ceoPlayerResult','click',async e=>{const b=e.target.closest('[data-ceo-action]');if(!b)return;const action=b.dataset.ceoAction;const username=b.dataset.username;try{const d=await post('/ceo/player/action',{username,action});toast(d.message,d.success?'success':'error');if(d.success)ceoSearch();}catch(err){toast(err.message,'error');}});
  $$('.auth-tab').forEach(b=>b.addEventListener('click',()=>switchAuth(b.dataset.auth)));
  $$('input,select').forEach(el=>el.addEventListener('change',()=>{if(el.id?.startsWith('set'))saveSettings();}));
  $$('.classic-count').forEach(b=>b.addEventListener('click',()=>startClassicSearch(Number(b.dataset.classicCount))));
...
async function enterApp(forceCustomize=false){
  ...
  renderMapPreview();renderAchievementsPreview();await loadMiniRank();connectSocket();if(state.user?.role==='CEO'){let b=$('#btnCeoHome');if(!b){b=document.createElement('button');b.id='btnCeoHome';b.className='menu-btn ceo-home';b.innerHTML='<span></span><b>CEO</b><small>Central administrativa</small>';$('#btnCustomize')?.after(b);b.onclick=()=>navigate('ceo');}}navigate('lobby');
...
function renderAchievementsPreview(){const el=$('#achievementPreview');if(!el)return;el.innerHTML=[['','Primeira Vitória'],['','Especialista'],['','Primeiro Online'],['','Colecionador']].map(a=>`<div class="achievement-chip"><span>${a[0]}</span><b>${a[1]}</b></div>`).join('');}
...
async function createRoom(){try{const body={name:$('#roomName')?.value||`Mesa de ${state.user.username}`,password:$('#roomPassword')?.value||'',maxPlayers:Number($('#roomMax')?.value||4),turnSeconds:Number($('#roomTime')?.value||45),difficulty:'classic',botFill:Number($('#roomBots')?.value||4),mapId:$('#roomMap')?.value||'map_saloon',startingCards:Number($('#roomCards')?.value||7),allowBots:$('#roomAllowBots')?.checked!==false,specials:$('#roomSpecials')?.checked!==false,stackDraw:$('#roomStack')?.checked===true,chat:$('#roomChat')?.checked!==false};const d=await post('/rooms',body);hide('#createRoomModal');if(!state.socket)connectSocket();state.socket?.emit('room:join',{code:d.roomCode,password:body.password});}catch(e){toast(e.message,'error');}}
function renderRoom(room){
  if(!room)return;$('#roomTitle')&&($('#roomTitle').textContent=room.name);$('#roomCodeBadge')&&($('#roomCodeBadge').textContent=room.code);$('#roomOptionsText')&&($('#roomOptionsText').textContent=`${room.players.length}/${room.options.maxPlayers} jogadores • ${room.options.turnSeconds}s • Clássico`);if($('#btnStartRoom'))$('#btnStartRoom').style.display=String(room.ownerId)===String(state.user.id)&&!room.started?'inline-flex':'none';
  ...
  const banner=$('#roomMapBanner');if(banner){banner.className=`room-map-banner map-${mapTheme(room.options.mapId)}`;banner.innerHTML=`<div><span> MAPA</span><b>${escapeHtml(MAPS.find(m=>m.id===room.options.mapId)?.name||room.options.mapId)}</b></div>`;}
}
...
async function startSolo(difficulty='classic',playerCount=1){state.solo=makeSolo(difficulty,playerCount);navigate('game');$('#arenaShell')?.classList.add('solo-arena');renderSolo();}
function startClassicSearch(count){cancelClassicSearch();state.classicCount=count;state.classicSearchStarted=true;navigate('classicSearch');renderCharacter('#classicSearchCharacter',state.profile.avatar);$('#classicSearchStatus').textContent='Procurando jogadores...';$('#classicSearchTitle').textContent=count===3?'Procurando trio':count===2?'Procurando duo':'Procurando partida';const max=20;const wait=3+Math.floor(Math.random()*(max-2));let left=wait;$('#classicSearchTimer').textContent=left+'s';state.classicTimer=setInterval(()=>{left--;if(left<=0){clearInterval(state.classicTimer);state.classicTimer=null;foundClassicMatch();}else $('#classicSearchTimer').textContent=left+'s';},1000);}
function foundClassicMatch(){if(!state.classicSearchStarted)return;navigator.vibrate?.([160,80,160]);try{if(window.speechSynthesis){speechSynthesis.cancel();const u=new SpeechSynthesisUtterance('Partida encontrada');u.lang='pt-BR';u.rate=.9;u.pitch=1.05;speechSynthesis.speak(u);}}catch{}$('#classicSearchStatus').textContent='Partida encontrada!';$('#classicSearchTimer').textContent='OK';const cd=$('#classicCountdown');if(cd)show(cd);let n=5;if(cd)cd.textContent=n;const t=setInterval(()=>{n--;if(cd)cd.textContent=n;if(n<=0){clearInterval(t);if(cd)hide(cd);state.classicSearchStarted=false;startSolo('classic',state.classicCount);}},1000);}
function cancelClassicSearch(){state.classicSearchStarted=false;if(state.classicTimer){clearInterval(state.classicTimer);state.classicTimer=null;}hide('#classicCountdown');}

function makeSolo(difficulty='classic',playerCount=1){const deck=makeDeck(),player=[],bot=[];for(let i=0;i<7;i++){player.push(deck.pop());bot.push(deck.pop());}let top=deck.pop();while(top.color==='black'){deck.unshift(top);top=deck.pop();}return{difficulty,playerCount,deck,player,bot,discard:top,pile:[],color:top.color,turn:'player',botName:'Oponente'};}
function renderSolo(){const g=state.solo;if(!g)return;$('#roundText')&&($('#roundText').textContent=g.playerCount===3?'TRIO':g.playerCount===2?'DUO':'SOLO');$('#turnStatus')&&($('#turnStatus').textContent=g.turn==='player'?'SUA VEZ!':'VEZ DO BOT');$('#turnStatus')?.classList.toggle('bot',g.turn!=='player');renderArenaCard(g.discard,g.color);$('#deckCount')&&($('#deckCount').textContent=g.deck.length);$('#opponents')&&($('#opponents').innerHTML=`<div class="opponent-card"><div class="opponent-avatar"></div><div><b>${escapeHtml(g.botName)}</b><small>${g.bot.length} cartas</small></div><div class="mini-hand">${Array.from({length:Math.min(7,g.bot.length)},()=>'<span class="back-mini">UNO</span>').join('')}</div></div>`);const hand=$('#playerHand');if(hand)hand.innerHTML=g.player.map((c,i)=>cardHtml(c,i,g.player.length)).join('');}
...
function botTurn(){const g=state.solo;if(!g||g.turn!=='bot')return;let cards=g.bot.filter(c=>playable(c,g.discard,g.color));cards.sort((a,b)=>cardScore(b)-cardScore(a));const card=cards[0];if(!card){drawSolo(g.bot,1);g.turn='player';renderSolo();return;}g.bot.splice(g.bot.indexOf(card),1);g.pile.push(g.discard);g.discard=card;g.color=card.color==='black'?chooseColorBot(g.bot):card.color;Sound.card();if(card.type==='draw2')drawSolo(g.player,2);if(card.type==='draw4')drawSolo(g.player,4);if(g.bot.length===0)return finishSolo(false);g.turn=card.type==='skip'||card.type==='reverse'?'bot':'player';renderSolo();if(g.turn==='bot')setTimeout(botTurn,800);}
...
function renderAchievements(){const el=$('#inventoryContent');if(!el)return;const arr=[['','Primeira Vitória','Vença sua primeira partida.'],['','Especialista','Vença partidas clássicas.'],['','Primeiro Online','Finalize uma partida online.'],['','Colecionador','Desbloqueie 10 itens.'],['⭐','Nível 10','Alcance o nível 10.'],['','Nível 25','Alcance o nível 25.']];el.innerHTML=`<div class="achievement-list">${arr.map(a=>`<div class="achievement-card"><span>${a[0]}</span><div><b>${a[1]}</b><small>${a[2]}</small></div><em>EM PROGRESSO</em></div>`).join('')}</div>`;}
...
async function openHistory(){navigate('history');try{const [h,st]=await Promise.all([get('/history'),get('/stats')]);const s=st.stats||{};$('#statsCards').innerHTML=[['SOLO',s.solo||0],['DUO',s.duo||0],['TRIO',s.trio||0],['VITÓRIAS',s.wins||0],['PARTIDAS',s.gamesPlayed||s.games||0]].map(x=>`<div class="stat-card"><b>${fmt(x[1])}</b><small>${x[0]}</small></div>`).join('');$('#historyRows').innerHTML=(h.matches||[]).map(m=>`<div class="history-row"><b>${escapeHtml(m.mode||'CLÁSSICO')}</b><span>${escapeHtml(m.result||'PARTICIPAÇÃO')}</span><small>${new Date(m.endedAt||m.startedAt).toLocaleString('pt-BR')}</small></div>`).join('')||'<div class="empty-state">Nenhuma partida registrada.</div>';}catch(e){toast(e.message,'error');}}
async function ceoSearch(){if(state.user?.role!=='CEO')return;const name=$('#ceoPlayerName')?.value?.trim();if(!name)return;try{const d=await get('/ceo/player?username='+encodeURIComponent(name));const p=d.player;$('#ceoPlayerResult').innerHTML=p?`<div class="ceo-player"><b>${escapeHtml(p.username)}</b><small>Nível ${p.level} · ${p.gamesPlayed} partidas · ${p.wins} vitórias · ${fmt(p.coins)} moedas · ${fmt(p.xp)} XP</small><div class="ceo-actions"><button data-ceo-action="clear_inventory" data-username="${escapeHtml(p.username)}">LIMPAR INVENTÁRIO</button><button data-ceo-action="clear_xp" data-username="${escapeHtml(p.username)}">ZERAR XP</button><button data-ceo-action="clear_coins" data-username="${escapeHtml(p.username)}">ZERAR MOEDAS</button><button data-ceo-action="ban" data-username="${escapeHtml(p.username)}">BANIR</button></div></div>`:'<div class="empty-state">Jogador não encontrado.</div>';}catch(e){toast(e.message,'error');}}
async function ceoCommand(command){if(state.user?.role!=='CEO')return;try{const d=await post('/ceo/command',{command});toast(d.message,d.success?'success':'error');}catch(e){toast(e.message,'error');}}
async function loadCeo(){if(state.user?.role!=='CEO')return;try{const d=await get('/ceo/season');$('#ceoSeasonInfo').textContent=`Temporada ${d.season.number} · termina em ${new Date(d.season.endsAt).toLocaleString('pt-BR')}`;$('#ceoSeasonDays').value=d.season.days||30;}catch{}}
async function saveSeason(){if(state.user?.role!=='CEO')return;try{const days=Math.max(1,Math.min(3650,Number($('#ceoSeasonDays').value)||30));const d=await post('/ceo/season',{days});toast(d.message,'success');loadCeo();}catch(e){toast(e.message,'error');}}
